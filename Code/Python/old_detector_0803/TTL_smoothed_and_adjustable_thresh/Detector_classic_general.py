...
CONFIG_PATH = "/mnt/hubel-data-103/Guillaume/InfraSlowRhythmLiveDetector/Code/Python/TTL_smoothed_and_adjustable_thresh/OE_classic_CONFIG.py"
...
class PyProcessor:
    ...
    def _load_config(self):
        ...
        required = [
            ...
            "bin_duration_accel_window","bin_real_wake_acc"
        ]
    ...
    def handle_ttl_event(self, source_node, channel, sample_number, line, state):
        ...
        if line == self.increase_threshold and state == True and self.thrincr == False:
            self.thrincr = True
            self.threshold_value += self.pas_thresh_ajust
            if self.phase2_done:
                self.threshold_value_sws += self.pas_thresh_ajust
        if line == self.increase_threshold and state == False:
            self.thrincr = False

        if line == self.decrease_threshold and state == True and self.thrdecr == False:
            self.thrdecr = True
            self.threshold_value -= self.pas_thresh_ajust
            if self.phase2_done:
                self.threshold_value_sws -= self.pas_thresh_ajust
    ...
    def process(self, data):
        ...
        # Smooth the acceleration over a 5-second window to prevent brief micro-awakenings from interfering with IS detection.(5 second is the length of bin_duration_accel_window define in the OE_classic_CONFIG.py)
        ...
        while self.real_wake_duration > self.bin_duration_accel_window:
            val = self.real_wake_window.popleft()
            dt = self.real_wake_time.popleft()
            self.real_wake_duration -= dt
        real_wake_acc=np.dot(np.array(self.real_wake_window, dtype=float), np.array(self.real_wake_time))>self.bin_real_wake_acc
        ...
        if real_wake_acc:
            self.processor.add_python_event(self.ttl_real_wake_acc, True)
            if not self.real_wake_acc_state:
                self.L_time_real_wake_acc.append(self.time_counter)
                with open(self.debug_path, "a") as f:
                    print(f"[REAL WAKE DETECTED] t={self.time_counter:.1f}s", file=f)
            self.real_wake_acc_state = True
        else:
            if self.real_wake_acc_state:
                self.L_time_real_wake_acc.append(self.time_counter)
                with open(self.debug_path, "a") as f:
                    print(f"[REAL WAKE ENDED] t={self.time_counter:.1f}s", file=f)
            self.real_wake_acc_state = False
    ...
    #Pour synchroniser arduino et python processor, on envoie un TTL de sync au début de l'acquisition
    ...
    def stop_acquisition(self):
        if self.saving:
            ...
            with open(namedossier, "a") as f:
                ...
                print("List of real wake accelerometer timing markers (starts with 'start'):", file=f)
                print(self.L_time_real_wake_acc, file=f)
                ...
                print("Final adjusted FR threshold:", file=f)
                print(self.threshold_value, file=f)
                if self.phase2_done:
                    print(self.threshold_value_sws, file=f)
