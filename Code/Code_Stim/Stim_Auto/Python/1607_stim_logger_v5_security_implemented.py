...
SYSEX_RESUME = bytes([0xF0, 0x7C, 0xF7])
...
def main():
    ...
    print()
    print("Commandes :")
    print("  START              : démarrer la session")
    print("  E | EXIT | P | PAUSE : arrêt d'urgence immédiat")
    print("  R | RESUME         : reprendre après une pause")
    print("  STOP | q           : arrêter la session et fermer")
    ...
    try:
        while running:
            ...
            if source == "stdin":
                ...
                if cmd in ("R", "RESUME"):
                    if not session_active:
                        ...
                        continue
                    if not emergency_paused:
                        print("  Le protocole n'est pas actuellement en pause.")
                        continue
                    try:
                        ser.write(SYSEX_RESUME)
                        ser.flush()
                        print("  → Reprise envoyée. Nouveau délai pré-stimulation.")
                    except Exception as e:
                        print(f"  Erreur envoi RESUME : {e}")
                    continue

                # ── ARRÊT COMPLET DU LOGGER ─────────────────────────────────────────────
                ...
                print(f"  Inconnu : '{line}' — commandes : START | E | EXIT | P | PAUSE | R | RESUME | STOP | q" )
                continue
            ...
            if event_key == "EMERGENCY_PAUSE":
                emergency_paused = True
                print("Stimulation en pause.")
                print("Le protocole a subi un arrêt d'urgence.")
                print('Pour reprendre, tapez "R" ou "RESUME".')

            elif event_key == "EMERGENCY_RESUME":
                emergency_paused = False
                print("Le protocole reprend.")
                print("Le bloc interrompu est abandonné.")
                print("Un nouveau délai pré-stimulation sera appliqué.")
                print()


    except KeyboardInterrupt:
        print("\nInterruption clavier — arrêt d'urgence envoyé.")
        try:
            ser.write(SYSEX_PAUSE)
            ser.flush()
        except Exception as e:
            print(f"Erreur pendant l'arrêt d'urgence : {e}")
    finally:
        # Toujours tenter de couper la stimulation avant de fermer la liaison série
        try:
            ser.write(SYSEX_PAUSE)
            ser.flush()
        except Exception:
