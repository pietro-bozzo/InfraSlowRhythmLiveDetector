...
// Synchronisation TTL depuis OE
bool syncReceived = false;  // devient true dès le premier TTL sync reçu
bool protocolPaused = false; // true quand PAUSE est actif
...
void stimulationLogic() {
  if (protocolPaused) return;  // protocole en pause → ne rien faire
    ...
    for (pin = port * 8; pin < lastPin; pin++) {
      if (IS_PIN_DIGITAL(pin)) {
        if (Firmata.getPinMode(pin) == OUTPUT || Firmata.getPinMode(pin) == INPUT) {
          ...
          // Premier front montant sur syncPin → lancer la session automatiquement
...
void sysexCallback(byte command, byte argc, byte *argv) {
  ...
  if (command == SYSEX_CMD_PAUSE && !protocolPaused) {
    protocolPaused = true;
    if (stimState == SS_BLOCK_ON) {
        digitalWrite(stimulationPin, LOW);
        logStimEnd(millis());
    }
    Serial.print("SESSION_PAUSE,");
    Serial.println(formatTime(millis() - sessionStart));
    return;
  }

  if (command == SYSEX_CMD_UNPAUSE && protocolPaused) {
    protocolPaused = false;
    // Repart en attente d'immobilité (bloc interrompu abandonné, nouveau bloc
    // sera tiré au prochain PRE_STIM_WAIT_MS comme après un réveil)
    sleepOnsetTime = millis();
    stimState      = SS_PRE_WAIT;
    Serial.print("SESSION_UNPAUSE,");
    Serial.println(formatTime(millis() - sessionStart));
    return;
  }
