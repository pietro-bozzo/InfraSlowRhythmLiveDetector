...
// 0x7F est dans la plage réservée "utilisateur" de la spec Firmata
#define SYSEX_CMD_START  0x7F
#define SYSEX_CMD_STOP   0x7E   // commande custom STOP
#define SYSEX_CMD_PAUSE  0x7D   // arrêt d'urgence
#define SYSEX_CMD_RESUME 0x7C   // reprise après arrêt d'urgence
...
// ── Paramètres anti-bruit réveil ─────────────────────────────────────────────
#define WINDOW_MS          7000UL   // fenetre d integration TTL (ms)
#define K_MS               1000UL   // seuil : TTL integre > K_MS -> pre_wake
#define PRE_WAKE_CONFIRM_MS 5000UL  // pre_wake continu >= 5s -> wake confirme
#define SLEEP_CONFIRM_MS   8000UL   // absence pre_wake >= 8s -> sleep confirme

// ── Définition des blocs ──────────────────────────────────────────────────────
// Bloc A : 4 durées mélangées, pause fixe 60 s après chaque stim
const unsigned long BLOCK_A_DURATIONS[4] = { 100, 150, 200, 300 };  // ms
const unsigned long BLOCK_A_OFF_MS       = 60000UL;
const uint8_t       BLOCK_A_REPS         = 4;

// Blocs B, C, D
const unsigned long BLOCK_B_ON_MS  = 100;   const unsigned long BLOCK_B_OFF_MS = 5000;   const uint8_t BLOCK_B_REPS = 10;
const unsigned long BLOCK_C_ON_MS  = 100;   const unsigned long BLOCK_C_OFF_MS = 10000;  const uint8_t BLOCK_C_REPS = 10;
const unsigned long BLOCK_D_ON_MS  = 100;   const unsigned long BLOCK_D_OFF_MS = 20000;  const uint8_t BLOCK_D_REPS = 10;
...
void emergencyPause(unsigned long now) {
  ...
  // Ne rien refaire si le protocole est déjà en pause
  if (emergencyPaused) {
    digitalWrite(stimulationPin, LOW);
    return;
  }
  
  // Ne pas créer de pause s’il n’y a aucune session en cours
  if (!sessionStarted || progPhase == PHASE_IDLE) {
    digitalWrite(stimulationPin, LOW);
    return;
  }
  ...
  // Si la pause survient pendant la baseline,
  // sauvegarder uniquement le temps écoulé avant la pause
  if (progPhase == PHASE_BASELINE && baselineLastOnset > 0) {
    ...
    for (pin = port * 8; pin < lastPin; pin++) {
      if (IS_PIN_DIGITAL(pin)) {
        if (Firmata.getPinMode(pin) == OUTPUT || Firmata.getPinMode(pin) == INPUT) {
          ...
          if (pin == instructionPin) ttlRaw = (pinValue == 1);
