...
%[t, acc, a1, a2, a3, acc_glissante, mean_acceleration_decay, threshold_line, threshold_acceleration] = plotAccelerationWithThreshold(session, [161 162 163], [0 10]);


[filebase,basename] = fileparts(session);

filename = '/mnt/hubel-data-103/Guillaume/InfraSlowRhythmLiveDetector/Output_oe/Tests_sessions_finaux/Tests_stim/Rat012/IS_wake_timings_2THRESHOLDS_debug012.txt';
...
us_intervals = R.eventIntervals("slownr","sleep#1", "regexp",true);      % InfraSlow Rhythm (MATLAB detection)
us_avals     = R.eventIntervals("slownr","sleep#1", "regexp",true);  % Avalanches
...
start_reccord_sec = 640; %1580; % Chosen cumulative session time (s) % Je crois avoir compris : sion regarde sleep1, c'est quand sleep 1 commence dans la session totale. Pas automatisable, ou alors en récupérant les temps deb fin de chaque recording node...
